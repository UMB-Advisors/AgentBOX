...
| 3 | On the box | `~/agentbox/install/demo-reset.sh --reboot` (run once with `--dry-run` first to preview the blast radius) | clears onboarding + connected accounts, takes Wi-Fi offline, reboots → ~90 s later broadcasts **`AgentBOX-Setup-XXXX`** |
...
| 5 | Re-demo | `~/agentbox/install/demo-reset.sh --reboot` → back to step 4 | fresh wizard again |

## Scenario B — box already provisioned (the common re-demo)

Skip steps 1–2.

1. **On the box:** `~/agentbox/install/demo-reset.sh --reboot`
   *(add `--wipe-mail-data` for a truly clean slate that also clears prior emails/drafts/RAG)*
2. **Phone:** same as Scenario A, step 4.
