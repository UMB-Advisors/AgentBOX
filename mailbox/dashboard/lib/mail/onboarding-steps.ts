// dashboard/lib/mail/onboarding-steps.ts
//
// MBOX-465 (child of MBOX-355 multi-provider mail) — provider-aware onboarding
// CONTENT layer for the Connections/Keys page. A non-technical operator connects
// their own Microsoft 365 or IMAP account from the UI; this module is the SINGLE
// SOURCE OF TRUTH for the inline walkthrough (Azure app-registration steps,
// IMAP/SMTP app-password recipes, host/port defaults) so the UI renders steps
// from DATA, never bespoke per-provider JSX.
//
// AC4 ("a new provider contributes its own steps with no UI fork") is enforced
// by typing PROVIDER_ONBOARDING as Record<MailProviderKind, ProviderOnboarding>:
// adding a value to MAIL_PROVIDERS (lib/types.ts) breaks the build here until the
// new provider supplies its onboarding entry — compile-time exhaustiveness.
//
// Keep operational constants (the Mail.ReadWrite permission name, host/port
// defaults) in DATA here, not in the consuming JSX, so a future scope change
// stays single-sourced. Field hints (`produces`) surface only snake_case names
// that match the connect-schema bodies (tenant_id, client_secret, app_password ...)
// so the UI can map a step to the input it populates.
//
// DATA-ONLY: no DB, no fetch, no React. The test-connection seam already exists
// (app/api/accounts/{microsoft,imap}/route.ts -> connect-{graph,imap}.ts); this
// module only declares which route each credentials provider posts to.

import type { MailProviderKind } from '@/lib/types';

// 'credentials' = operator pastes connection params + an app secret/password and
// runs the Test-connection probe (microsoft, imap). 'oauth' = native OAuth grant
// wired separately (gmail) — no inline credential steps.
...
export interface ProviderOnboarding {
  ...
  summary: string;
  mode: OnboardingMode;
  // '/api/accounts/microsoft' | '/api/accounts/imap'. Undefined for oauth (gmail).
  connectPath?: string;
  // Ordered inline walkthrough. [] for oauth providers.
...
// (lib/mail/test-graph-connection.ts:graphMailboxVerdict — "grant the app the
// Mail.ReadWrite APPLICATION permission and admin consent"). One scope change
// here, not scattered across step strings.
const GRAPH_PERMISSION = 'Mail.ReadWrite';

const GMAIL_ONBOARDING: ProviderOnboarding = {
  label: 'Gmail',
  summary:
    'Gmail connects through Google OAuth — there are no app credentials to paste here. Use the Google connect button to authorize the appliance, then it appears as a connected account.',
  mode: 'oauth',
  steps: [],
};
...
const IMAP_ONBOARDING: ProviderOnboarding = {
  ...
  steps: [
    ...
    },
    {
      ...
      body: 'Run Test connection — it does a live IMAP login and SMTP auth against the credentials. A green result is required before Save; a failure shows the exact server error so you can fix host, port, or password.',
    },
  ],
  imapPresets: [
    {
...
const MICROSOFT_ONBOARDING: ProviderOnboarding = {
  ...
  steps: [
    ...
    {
      title: 'Save',
      body: 'Save is enabled only after a passing test. Saving persists the account and encrypts the client secret at rest; the secret is never shown again.',
    },
  ],
};

// AC4 guarantee: Record key set is MailProviderKind, so the build fails if a new
// MAIL_PROVIDERS entry lands without its onboarding content here. Do NOT widen
// the key type to string — that would silently drop the exhaustiveness check.
export const PROVIDER_ONBOARDING: Record<MailProviderKind, ProviderOnboarding> = {
  gmail: GMAIL_ONBOARDING,
