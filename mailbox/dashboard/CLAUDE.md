...
> **⚠️ Status (2026-06-12, MBOX-469): this is the headless MailBox pipeline BACKEND, not "the dashboard."** The operator UI = the AgentBOX dashboard served by **agentbox-sidecar** (`:9200`, tunnel `:9120`; repo `UMB-Advisors/agentbox-sidecar` — UI in `web/`, backend routes in the sidecar FastAPI app). `hermes-agent-main/.../web` is a **stale duplicate — never build there**. Under MBOX-469 every operator-facing page here was ported off; this Next.js service persists **only** as the backend behind the hermes reverse-proxy (`/dashboard/*` → `:3001`, the single agentbox2-v3 hermes patch — dies with the retirement endgame), serving n8n's ~33 `/api/internal/**` routes (token mint, inbox-messages, classify/draft, gmail bootstrap/cooldown, digest, imap-credentials, …) + proxied JSON. **Do not build new operator UI here.** The `mailbox-dashboard` docker service name is a load-bearing DNS name hardcoded in 8 n8n workflows; do not rename it. See the `[STATE]` comment on MBOX-469.
