...
- **server.js** (single file, ~200 lines):
  ...
  - Strip hop-by-hop headers; rewrite nothing else (Hermes UI is same-origin under /b/agentbox2/ — if absolute-path assets break, mount at ROOT per-box instead: one Railway service per box is acceptable for PoC; document whichever holds).
...
- **Deploy:** `railway init`/link a new service (project name mbox-498-relay-poc), set `BOX_TOKENS=agentbox2:<openssl rand -hex 32>`, `railway up`, get public URL (`railway domain`). Store the token ONLY in Railway env + a local scratch env file outside the repo (scratchpad), path noted in transcript.
- Probe from public side via ctx_execute fetch: healthz 200; /b/agentbox2/ with no key → 401; with key but no tunnel yet → 503 (expected until Phase 3).
...
- /b/agentbox2/ without token → 401; with token (no tunnel) → 503
