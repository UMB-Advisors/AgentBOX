...
## Goals
1. Prove the **vendor fleet plane**: agentbox2 enrolled as a *tagged* device (`tag:box`) via scoped auth key, vendor SSH/OTA access surviving, isolation expressed via grants — the $1/device/mo model from the research brief.
...
- agentbox2's existing services must survive: sidecar :9200, Tailscale Serve :9120, public Funnel :443→:8088 (don't touch).
...
## Memory hits applied
- agentbox2-access: exact endpoints (ssh UMB@agentbox2, :9200 sidecar, :9120 Serve, Funnel :443 hands-off).
